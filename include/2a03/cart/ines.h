...
namespace iNESv1
{
	...
	public:
		...
		{
			std::fill(
				trainer.get(), trainer.get() + trainer_sz, 0x0);
			std::fill(
				prg_rom.get(), prg_rom.get() + trainer_sz, 0x0);
			std::fill(
				chr_rom.get(), chr_rom.get() + trainer_sz, 0x0);
			std::fill(
				prg_ram.get(), prg_ram.get() + trainer_sz, 0x0);
		};
