...
namespace NES
{
	...
	{
		bitfield_union(Byte2, uint8_t value,
			bool pal_sel_l : 1;	///< Palette select low bit.
			bool pal_sel_h : 1;	///< Palette select high bit.
			bool RESERVED : 3;
			bool obj_pri : 1;	///< Object priority: > playfield / < playfield (0/1).
			bool bit_reverse : 1;	///< Apply bit reversal to fetched object pattern table data.
			bool inv_scan_addr : 1; ///< Invert the 3/4-bit (8/16 scanlines/object mode) scanline address used to access an object tile.
		);
		
		uint8_t coord;		///< Scanline coordinate minus one of object's top pixel row.
		uint8_t tile_idx; 	///< Tile index number. Bit 0 controls pattern table selection when r0.5 == 1.
		Byte2 b2;
		uint8_t scan_pix_coord; ///< Scanline pixel coordinate of most left-hand side of object.
